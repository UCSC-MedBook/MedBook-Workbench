	...
	Meteor.methods({
		...
		violinplot_adapter: function (argList) {
				...
				var whendone = function(retcode, workDir, contrastId, contrastName, studyID, uid) {
					...
					console.log('whendone work dir', workDir, 'return code', retcode, 'user id', uid);
					try {
						buf = fs.readFileSync(path.join(workDir,'report.list'), {encoding:'utf8'}).split('\n');
					}
					catch (err){
		...
		isoformplot_adapter: function (argList) {
				console.log('user', this.userId);
				console.log('isoform plot adapter',argList);
				read_config();
				var contrastId = argList[0];
				var sampleList =  {'_id':0};
				var gene = argList[1];
				...
				var cmd = medbook_config.tools.isoformplot.path;
				var whendone = function(retcode, workDir, contrastId, contrastName, studyID, uid) {
					var idList = [];
					var buf;
					console.log('whendone work dir', workDir, 'return code', retcode, 'user id', uid);
		...
		gsea_adapter: function (argList) {
			console.log('user', this.userId);
			console.log('gsea_adapter',argList);
			read_config();
			var signatureId = argList[0];
			//var signatureId = ObjectId("0fe9105aa1c7d24f9e209c4a")
			var sampleList =  {'_id':0};
			//workDir = '/private/var/tmp/limmaTmp'
			workDir = ntemp.mkdirSync('gseaWork');
			var sigfile =path.join(workDir, 'sig.tab');
			console.log('gsea_adapter sig',signatureId, sigfile);
			var signature = Signature.findOne({"name" : "Small Cell vs Non Small Cell"},{list1:1,'name':1,'studyID':1,_id:0});
			var signatureName;
			try {
				signatureName = signature.name;
			}
			catch(error) {
				console.log('No signature found for ', argList, " error is ", error);
				return -1;
			}
			var studyID = signature.studyID;
			var wstream = fs.createWriteStream(sigfile);
			//wstream.write( "gene\tscore\n");
			console.log('# of samples in each side of' , signature.name,': ' , signature.signature);
	_.map(signature.signature, function(value, item) {
				//	_.each(signature['signature'], function(item) {
				var wt = value.weight.toString();
				//console.log('item', item, 'value', value, wt);
				wstream.write(item);
				sampleList[item] = 1;
				wstream.write('\t');
				wstream.write(wt);
			...
			wstream.end();
			var genesetfile =path.join(workDir, 'genesets.gmt');

			console.log('input files', genesetfile, sigfile);
			var geneset_curs = GeneSets.find({});
			var fd = fs.openSync(genesetfile,'w');
			console.log('geneset count' , geneset_curs.count());

			geneset_curs.forEach(function(geneset) {

				fs.writeSync(fd,geneset.gene);
				fs.writeSync(fd,'\t');
				fs.writeSync(fd,geneset.gene);
				fs.writeSync(fd,'\t');
				_.map(geneset.memebers, function(member) {

						fs.writeSync(fd,member);
						fs.writeSync(fd,'\t');
				});
				fs.writeSync(fd,'\n');
			});
			fs.closeSync(fd);
			fs.exists(genesetfile, function(data) {
				console.log('file',	 genesetfile, 'exists?', data );
			});

			var cmd = medbook_config.tools.gsea.path;
			var whendone = function(retcode, workDir, signatureId, signatureName, studyID, uid) {
				var idList = [];
				console.log('whendone work dir', workDir, 'return code', retcode, 'user id', uid);
				var buf = fs.readFileSync(path.join(workDir,'report.list'), {encoding:'utf8'}).split('\n');
				_.each(buf, function(item) {
		...
		//input1=$1 idcol=$2 outdir=$3 gmtSource=$4  name=$5
			Meteor.call('runshell', cmd, [sigfile, genesetfile, 'outdir', 'name'],
				workDir, signatureId, signatureName, studyID, path.join(workDir,'report.list'), whendone, function(err,response) {
					if(err) {
						console.log('serverDataResponse', "gsea_adapter Error:" + err);
						return ;
					}
			resultObj = response.stderr;
			console.log('gsea started stdout stream id: '+resultObj._id+ ' stdout name '+resultObj.name());
			var readstream = resultObj.createReadStream('blobs');
			readstream.setEncoding('utf8');
			readstream.on('data', function(chunk) {
				console.log('chunk', chunk);
